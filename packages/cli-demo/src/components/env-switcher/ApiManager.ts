      ...
      case 'production':
        return {
          ...
          headers: {
            ...
            'X-Version': (() => {
              try {
                return (typeof process !== 'undefined' && process.env) ? process.env.REACT_APP_VERSION || '1.0.0' : '1.0.0';
              } catch {
                return '1.0.0';
              }
            })()
...
export const useApi = () => {
  try {
    ...
    // 返回空的API对象，避免错误
    return {
      get: async () => ({ message: 'API未初始化' }),
      post: async () => ({ message: 'API未初始化' }),
      put: async () => ({ message: 'API未初始化' }),
      delete: async () => ({ message: 'API未初始化' }),
      request: async () => ({ message: 'API未初始化' }),
      healthCheck: async () => ({ 
      ...
      getConfig: () => ({ 
        ...
        timeout: 0, 
        headers: {}, 
        environment: { name: 'unknown' } as any 
      }),
      setMockMode: () => {},
      isInitialized: false
    };
