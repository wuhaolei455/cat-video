...
const LeoVideoPlayer = react.forwardRef(({ config, width = '100%', height = 'auto', className = '', style = {}, showControls = true, showEventLog = false, onStateChange, onEvent, onError }, ref) => {
    const videoRef = react.useRef(null);
    const playerRef = react.useRef(null);
    const [playerState, setPlayerState] = react.useState(defaultPlayerState$1);
    const [eventLogs, setEventLogs] = react.useState([]);
    // 处理视频事件
    const handleVideoEvent = react.useCallback((type, data) => {
        // 记录事件日志
        if (showEventLog) {
            const logEntry = {
    ...
    return (jsxRuntime.jsxs("div", { className: `leo-video-player ${className}`, style: { width, height, ...style }, children: [jsxRuntime.jsx("video", { ref: videoRef, controls: showControls, style: { width: '100%', height: '100%' }, playsInline: true }), showEventLog && (jsxRuntime.jsxs("div", { className: "leo-video-events", style: {
                    marginTop: '10px',
                    maxHeight: '200px',
                    overflow: 'auto',
                ...
                }, children: [jsxRuntime.jsx("h4", { children: "Event Log:" }), eventLogs.map(log => (jsxRuntime.jsxs("div", { style: { marginBottom: '5px' }, children: [jsxRuntime.jsx("span", { style: { color: '#666' }, children: new Date(log.timestamp).toLocaleTimeString() }), ' ', jsxRuntime.jsx("strong", { children: log.type }), ' ', jsxRuntime.jsx("span", { style: { color: '#888' }, children: JSON.stringify(log.data) })] }, log.id)))] }))] }));
});
...
const useLeoVideo = ({ config, autoInit = true, onStateChange, onEvent, onError }) => {
    const videoRef = react.useRef(null);
    const playerRef = react.useRef(null);
    const [playerState, setPlayerState] = react.useState(defaultPlayerState);
    ...
    const setQuality = react.useCallback((quality) => {
        if (!playerRef.current)
            throw new Error('Player not initialized');
        playerRef.current.setQuality(quality);
    }, []);
    ...
    // 状态查询方法
    const getAvailableQualities = react.useCallback(() => {
        if (!playerRef.current)
            return ['auto'];
    ...
    const isEnded = react.useCallback(() => {
        return playerState.state === 'ended';
    }, [playerState.state]);
    const isFullscreen = react.useCallback(() => {
        return playerState.isFullscreen;
    }, [playerState.isFullscreen]);
    const isPictureInPicture = react.useCallback(() => {
...
exports.HTML5VideoAPI = HTML5VideoAPI;
exports.LeoVideoPlayer = LeoVideoPlayer;
exports.PresetConfigFactory = PresetConfigFactory;
exports.VideoConfigBuilder = VideoConfigBuilder;
exports.VideoEventEmitter = VideoEventEmitter;
exports.VideoEventEmitterTyped = VideoEventEmitterTyped;
exports.VideoPlayerFactory = VideoPlayerFactory;
exports.assertVideoConfig = assertVideoConfig;
exports.createSmartVideoPlayer = createSmartVideoPlayer;
exports.createVideoConfig = createVideoConfig;
exports.default = LeoVideoPlayer;
exports.detectDASHSupport = detectDASHSupport;
exports.detectHLSSupport = detectHLSSupport;
exports.detectVideoFormats = detectVideoFormats;
exports.isVideoState = isVideoState;
exports.useLeoVideo = useLeoVideo;
